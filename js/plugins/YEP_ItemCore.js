...
Game_Actor.prototype.setup = function(actorId) {
    ...
    $gameSwitches.setValue(457, true);
    this.initIndependentEquips($dataActors[actorId].equips);
    $gameSwitches.setValue(457, false);
};
...
Game_Party.prototype.gainItem = function(item, amount, includeEquip) {
	var oldamount = $gameParty.numItems(item);
    if (DataManager.isIndependent(item)) {
    var amountw = amount
    
    if(amount < 0){
    if($gameParty.numIndependentItems(item) < (-1 * amount)){
    amountw = -1 * $gameParty.numIndependentItems(item);
    if(amountw == 0) amountw = -1;
    }
    }
    if($gameWeight) amountw = 0;
    $gameWeight = false;
        var weight = 0;
      if(amountw != 0 && item && $gameSwitches.value(348) && !$gameSwitches.value(457)){
      if(item.meta.Weight){

      var www = item.meta.Weight * 1 * amountw;
      weight =  $gameVariables.value(146) + www;
      $gameVariables.setFloatValue(146, weight);
      }
      }
      ...
      var newamount = $gameParty.numItems(item);
      var sumamount = newamount - oldamount
      var weight = 0;
      if(item && $gameSwitches.value(348) && !$gameSwitches.value(457)){
      if(item.meta.Weight){
      var www = item.meta.Weight * 1 * sumamount
      weight =  $gameVariables.value(146) + www;
      $gameVariables.setFloatValue(146, weight);
      }
      }
...
Game_Variables.prototype.setFloatValue = function(variableId, value) {
...
Window_ItemList.prototype.drawItemCarryNumber = function(item, dx, dy, dw) {
    ...
    if(index > $gameVariables.value(147)) $gameVariables.setValue(147, index);
...
Window_ItemStatus.prototype.drawLargeIcon = function() {
    ...
    var dy = (Window_Base._faceHeight - dh) / 2-10;
...
Window_ItemStatus.prototype.drawEquipInfo = function(item) {
    ...
    for (var i = 0; i < 8; ++i) {
      ...
      this.changeTextColor(this.systemColor());
      this.drawText(TextManager.param(i), dx, rect.y-7, dw);
      ...
      this.drawText(text, dx, rect.y-7, dw, 'right');
      this.changePaintOpacity(true);
    }
};
...
Window_ItemStatus.prototype.drawItemInfo = function(item) {
    ...
    for (var i = 0; i < 8; ++i) {
      ...
      this.drawText(text, dx, rect.y-7, dw);
      this.drawItemData(i, dx, rect.y-7, dw);
    }
};
...
Window_ItemInfo.prototype.drawItemInfo = function(dy) {
    ...
    var item = this._item
    var ddw = dw - 60
    this.drawItemName(item, dx, dy, ddw);
    var baseItem = DataManager.getBaseItem(item);
    var num = $gameParty.numIndependentItems(baseItem);
    //this.drawText(num, dx, dy, dw, 'right');
...
Window_ItemActionCommand.prototype.initialize = function(x, y) {
    this._windowHeight = Graphics.boxHeight - y + 8;
...
Scene_Item.prototype.onItemCancel = function() {
    ...
    $gameSwitches.setValue(242,true);
