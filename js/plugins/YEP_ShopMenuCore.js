...
Window_ShopCommand.prototype.createCommand = function(command) {
    ...
    if (command === 'BUY' && $gameVariables.value(148)!=0) {
      this.addCommand(TextManager.buy, 'buy');
    } else if (command === 'SELL') {
        if($gameVariables.value(148)!=0){
      this.addCommand(TextManager.sell, 'sell', !this._purchaseOnly);
      }else{
      this.addCommand("廃棄", 'sell', !this._purchaseOnly);
      }
...
Window_ShopInfo.prototype.drawLargeIcon = function() {
    ...
    var dy = (Window_Base._faceHeight - dh) / 2-7;
...
Window_ShopStatus.prototype.updateParamSwitch = function() {
    if (!this.isEquipItem()) return;
    if (!$gameVariables.value(155)) return false;
    if (this.isTouched(-1) || this.getInput('pageup')) {
      SoundManager.playCursor();
      this.adjustLeft();
      this.refresh();
    } else if (this.isTouched(1) || this.getInput('pagedown')) {
      SoundManager.playCursor();
      this.adjustRight();
      this.refresh();
